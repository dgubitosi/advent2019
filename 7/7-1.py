...
class IntCodeProcessor(object):
    ...
    def printDebug(self, array):
        if self.interactive and self.debug:
            string = ""
            for s in array:
                string += str(s) + " "
            print(string[:-1])
    ...
    def opcode03(self):
        ...
        if self.interactive:
            ...
            self.setInput(self.inputs.pop(0))
...
if __name__ == "__main__":
    ...
    def permute(s):
        ...
        for i, c in enumerate(s):
            perms += [ c + p for p in permute(s[:i]+s[i+1:])]
        return perms
    ...
    for p in permutations:
        out = 0
        for n in p:
            thruster = IntCodeProcessor(file=file, inputs=[ n, out ])
            thruster.run()
            out = thruster.getOutput()
        test[p] = out
